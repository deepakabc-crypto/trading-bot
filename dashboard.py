...
<div class="section">
  ...
  <form id="globalForm" onsubmit="return saveGlobal(event)">
    ...
    <div class="btn-row">
      <button type="submit" class="btn btn-g">💾 Save Global</button>
    </div>
  </form>
</div>
...
function renderIdx(prefix,d,cfg){
  ...
  const pl=document.getElementById(prefix+'Pos');
  pl.innerHTML=d.positions&&d.positions.length?d.positions.map(x=>'<div>'+x+'</div>').join(''):'<div style="color:#8b949e">No positions</div>';
  if(cfg){
  ...
  const tl=document.getElementById(prefix+'Trades');
  if(d.trades_today&&d.trades_today.length){
    tl.innerHTML=d.trades_today.map(t=>'<div>'+(t.leg||'')+' '+(t.strike||'')+' '+(t.action||'')+' '+(t.success?'✅':'❌')+'</div>').join('');
  }else{tl.innerHTML='<div style="color:#555">—</div>';}
}

function refresh(){
  fetch('/api/status').then(r=>r.json()).then(d=>{
    const cp=document.getElementById('combinedPnl');
    ...
    const ci=d.config?d.config.indices:{};
...
function saveGlobal(ev){
  ev.preventDefault();
  const et=document.getElementById('gEntry').value.split(':');
  const xt=document.getElementById('gExit').value.split(':');
  ...
  }).then(r=>r.json()).then(d=>alert(d.success?'Saved!':'Error: '+d.error));
  return false;
}

function saveIndex(ev,idx){
  ev.preventDefault();
  const p=idx.toLowerCase();
  const data={
  ...
  fetch('/api/config/'+idx,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(data)})
    .then(r=>r.json()).then(d=>alert(d.success?idx+' config saved!':'Error: '+d.error));
  return false;
}

function emergExit(idx){
  if(confirm('⚠️ Close ALL '+idx+' positions?')){
    fetch('/api/emergency-exit/'+idx,{method:'POST'}).then(r=>r.json()).then(d=>alert(d.message||JSON.stringify(d)));
  }
}
function emergExitAll(){
  if(confirm('⚠️ EMERGENCY: Close ALL positions on ALL indices?')){
    fetch('/api/emergency-exit-all',{method:'POST'}).then(r=>r.json()).then(d=>alert(d.message||JSON.stringify(d)));
  }
}
