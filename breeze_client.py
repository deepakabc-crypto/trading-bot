...
Multi-index support: NIFTY (NFO) + SENSEX (BFO)
Handles connection, orders, quotes with retry logic.
"""
...
class BreezeClient:
    ...
    def format_date(dt=None):
        """DD-Mon-YYYY format required by Breeze API."""
        if dt is None:
            dt = datetime.now(IST)
    ...
    def get_nearest_expiry(index_name='NIFTY'):
        ...
        expiry_day = 3 if index_name == 'NIFTY' else 4  # Thu=3, Fri=4
    ...
    def get_spot_price(self, index_cfg):
        """Get spot price for any index via IndexConfig."""
        try:
            # Primary attempt
            quotes = self._retry(
