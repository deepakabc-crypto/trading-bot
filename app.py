...
MIN_PREMIUM = 5  # Lowered to allow more trades
CHARGES_PER_LOT = 100
CHECK_INTERVAL = 30
...
def get_quantity():
    """Get total quantity = lots * lot_size (65)"""
    return get_lots() * QUANTITY

def get_min_premium():
    data = load_data()
    return data.get("min_premium", MIN_PREMIUM)
...
def load_data():
    ...
    return {
        ...
        "lots": 1,
        "min_premium": 5  # Minimum premium to take trade
    }
...
def get_summary():
    ...
    return {
        ...
        "min_premium": data.get("min_premium", MIN_PREMIUM)
    }
...
class BreezeAPI:
    ...
    def place_order(self, strike, option_type, expiry, quantity, side, price, retries=3):
        if not self.connected:
        ...
        for attempt in range(retries):
            try:
                logger.info(f"📝 Placing order (attempt {attempt+1}): {side} {quantity} NIFTY {strike} {option_type} @ ₹{price}")
                ...
                if order and order.get('Success'):
                    order_id = order['Success'].get('order_id', 'OK')
                    logger.info(f"✅ Order placed: {side} {strike} {option_type} - ID: {order_id}")
                    return order_id
                else:
                    error = order.get('Error', 'Unknown') if order else 'No response'
                    logger.error(f"❌ Order failed (attempt {attempt+1}): {error}")
                    if attempt < retries - 1:
                        time.sleep(1)
                        continue
        ...
        telegram.send(f"❌ Order FAILED after {retries} attempts:\n{side} {strike} {option_type}")
        return None
...
class IronCondor:
    ...
    def enter(self, spot, expiry):
        ...
        min_prem = get_min_premium()
        
        if credit < min_prem:
            logger.warning(f"⚠️ IC: Premium too low ({credit:.0f} < {min_prem})")
            return False
        ...
        # Place all 4 orders with delay between them
        order1 = self.api.place_order(sc, "call", expiry, qty, "sell", sc_p)
        time.sleep(0.5)
        order2 = self.api.place_order(bc, "call", expiry, qty, "buy", bc_p)
        time.sleep(0.5)
        order3 = self.api.place_order(sp, "put", expiry, qty, "sell", sp_p)
        time.sleep(0.5)
        order4 = self.api.place_order(bp, "put", expiry, qty, "buy", bp_p)
        ...
        failed_orders = [name for name, status in orders_status.items() if status is None]
        
        if failed_orders:
            logger.error(f"❌ IC Entry incomplete! Failed: {failed_orders}")
            telegram.send(f"⚠️ <b>IC Entry INCOMPLETE!</b>\nFailed legs: {', '.join(failed_orders)}\n\n⚠️ CHECK YOUR POSITIONS MANUALLY!")
    ...
    def exit(self, reason):
        ...
        # Place exit orders with delays
        order1 = self.api.place_order(self.position["sc"], "call", self.position["expiry"], qty, "buy", sc)
        time.sleep(0.5)
        order2 = self.api.place_order(self.position["bc"], "call", self.position["expiry"], qty, "sell", bc)
        time.sleep(0.5)
        order3 = self.api.place_order(self.position["sp"], "put", self.position["expiry"], qty, "buy", sp)
        time.sleep(0.5)
        order4 = self.api.place_order(self.position["bp"], "put", self.position["expiry"], qty, "sell", bp)
        
        # Check for failed orders
        orders_status = {"Buy SC": order1, "Sell BC": order2, "Buy SP": order3, "Sell BP": order4}
        failed_orders = [name for name, status in orders_status.items() if status is None]
        
        if failed_orders:
            telegram.send(f"⚠️ <b>IC Exit INCOMPLETE!</b>\nFailed: {', '.join(failed_orders)}\n\n⚠️ CHECK POSITIONS MANUALLY!")
...
class ShortStraddle:
    ...
    def enter(self, spot, expiry):
        ...
        if total < min_prem:
            logger.warning(f"⚠️ Straddle: Premium too low ({total:.0f} < {min_prem})")
            return False
        ...
        # Place orders with delay
        order1 = self.api.place_order(atm, "call", expiry, qty, "sell", ce)
        time.sleep(0.5)
        order2 = self.api.place_order(atm, "put", expiry, qty, "sell", pe)
        
        # Check for failed orders
        if not order1 or not order2:
            failed = []
            if not order1: failed.append("Sell CE")
            if not order2: failed.append("Sell PE")
            telegram.send(f"⚠️ <b>Straddle Entry INCOMPLETE!</b>\nFailed: {', '.join(failed)}\n\n⚠️ CHECK POSITIONS MANUALLY!")
    ...
    def exit(self, reason):
        ...
        # Place exit orders with delay
        order1 = self.api.place_order(self.position["strike"], "call", self.position["expiry"], qty, "buy", ce)
        time.sleep(0.5)
        order2 = self.api.place_order(self.position["strike"], "put", self.position["expiry"], qty, "buy", pe)
        
        # Check for failed orders
        if not order1 or not order2:
            failed = []
            if not order1: failed.append("Buy CE")
            if not order2: failed.append("Buy PE")
            telegram.send(f"⚠️ <b>Straddle Exit INCOMPLETE!</b>\nFailed: {', '.join(failed)}\n\n⚠️ CHECK POSITIONS MANUALLY!")
    ...
    <div class="container">
        ...
        <div class="section">
            ...
            <div class="grid" style="margin-top: 15px;">
                ...
                <div class="card">
                    <div class="card-title">Min Premium (₹)</div>
                    <input type="number" id="min-premium" class="time-input" value="5" min="0" max="100">
                </div>
            </div>
            <button class="btn" style="margin-top: 15px;" onclick="updateLots()">💾 Save Settings</button>
        </div>
    ...
    <script>
        ...
        async function refreshData() {
            try {
                ...
                document.getElementById('min-premium').value = data.min_premium || 5;
        ...
        async function updateLots() {
            ...
            const minPremium = parseInt(document.getElementById('min-premium').value) || 5;
            ...
            await fetch('/api/min_premium', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ min_premium: minPremium })
            });
            
            alert('Settings saved! Lots: ' + lots + ', Min Premium: ₹' + minPremium);
            refreshData();
        }
...
@app.route('/api/min_premium', methods=['POST'])
def api_min_premium():
    data = load_data()
    req = request.json
    
    min_prem = int(req.get("min_premium", 5))
    if min_prem < 0:
        min_prem = 0
    if min_prem > 100:
        min_prem = 100
    
    data["min_premium"] = min_prem
    save_data(data)
    
    telegram.send(f"📊 Min Premium updated: ₹{min_prem}")
    return jsonify({"status": "success", "min_premium": min_prem})
