...
def test_quotes():
    ...
    atm = round(spot / 50) * 50
    expiry = api.get_expiry()
    expiry_display = api.get_expiry_display()
    
    # Try different formats for option quotes
    results = {
        "spot": spot,
        "atm": atm,
        "expiry_api": expiry,
    ...
    try:
        ...
        results["tests"].append({
            "test": "Standard (call, NFO)",
            "expiry": expiry,
            "strike": atm,
            "response": ce_data
        })
    except Exception as e:
        results["tests"].append({"test": "Standard", "error": str(e)})
    
    # Test 2: Try with different expiry format
    try:
        exp2 = get_ist_now()
        days = (3 - exp2.weekday()) % 7
        if days == 0 and exp2.hour >= 15:
            days = 7
        exp2 = exp2 + timedelta(days=days)
        exp2_str = exp2.strftime("%d-%b-%Y")  # Like "13-Feb-2026"
        
        ce_data2 = api.breeze.get_quotes(
            stock_code="NIFTY",
            exchange_code="NFO",
            expiry_date=exp2_str,
            product_type="options",
            right="call",
            strike_price=str(atm)
        )
        results["tests"].append({
            "test": "Alt expiry format (DD-Mon-YYYY)",
            "expiry": exp2_str,
            "response": ce_data2
        })
    except Exception as e:
        results["tests"].append({"test": "Alt format", "error": str(e)})
