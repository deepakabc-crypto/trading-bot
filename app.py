...
class IronCondor:
    ...
    def get_live_pnl(self):
        ...
        try:
            ...
            pnl = (self.entry_premium - current_premium) * QUANTITY
...
<body>
    <div class="container">
        ...
        <div class="section" id="set-position-section">
            <div class="section-title">📝 Set Position Manually (Recovery)</div>
            <p class="info-text">Use this if bot restarted and you have an open position</p>
            <div class="grid" style="margin-top: 15px;">
                <div class="card">
                    <div class="card-title">Sell Call Strike</div>
                    <input type="number" id="pos-sc" class="time-input" placeholder="25950">
                </div>
                <div class="card">
                    <div class="card-title">Buy Call Strike</div>
                    <input type="number" id="pos-bc" class="time-input" placeholder="26050">
                </div>
                <div class="card">
                    <div class="card-title">Sell Put Strike</div>
                    <input type="number" id="pos-sp" class="time-input" placeholder="25650">
                </div>
                <div class="card">
                    <div class="card-title">Buy Put Strike</div>
                    <input type="number" id="pos-bp" class="time-input" placeholder="25550">
                </div>
                <div class="card">
                    <div class="card-title">Entry Premium (Net Credit)</div>
                    <input type="number" id="pos-premium" class="time-input" placeholder="28">
                </div>
                <div class="card">
                    <div class="card-title">Spot at Entry</div>
                    <input type="number" id="pos-spot" class="time-input" placeholder="25800">
                </div>
            </div>
            <button class="btn btn-success" style="margin-top: 15px;" onclick="setPosition()">✅ Set Iron Condor Position</button>
        </div>
    ...
    <script>
        ...
        async function refreshData() {
            try {
                ...
                const setPosSection = document.getElementById('set-position-section');
                
                if (data.current_position) {
                    posSection.style.display = 'block';
                    setPosSection.style.display = 'none';  // Hide manual entry when position exists
                    ...
                    setPosSection.style.display = 'block';  // Show manual entry when no position
                }
        ...
        async function setPosition() {
            const sc = parseInt(document.getElementById('pos-sc').value);
            const bc = parseInt(document.getElementById('pos-bc').value);
            const sp = parseInt(document.getElementById('pos-sp').value);
            const bp = parseInt(document.getElementById('pos-bp').value);
            const premium = parseFloat(document.getElementById('pos-premium').value);
            const spot = parseFloat(document.getElementById('pos-spot').value);
            
            if (!sc || !bc || !sp || !bp || !premium) {
            ...
            alert('Position set! Bot will now track this position.');
            
            // Clear input fields
            document.getElementById('pos-sc').value = '';
            document.getElementById('pos-bc').value = '';
            document.getElementById('pos-sp').value = '';
            document.getElementById('pos-bp').value = '';
            document.getElementById('pos-premium').value = '';
            document.getElementById('pos-spot').value = '';
...
def api_set_position():
    """Manually set a position for recovery"""
    data = load_data()
    req = request.json
    ...
    if strategy == "IRON_CONDOR":
        position = {
            "strategy": "IRON_CONDOR",
            "entry_time": req.get("entry_time", get_ist_now().strftime("%Y-%m-%d %H:%M:%S")),
            "spot": req.get("spot", 0),
            "strikes": {
                "sc": req.get("sc", 0),
                "bc": req.get("bc", 0),
            ...
            "premiums": {
                "sc": req.get("sc_premium", 0),
                "bc": req.get("bc_premium", 0),
                "sp": req.get("sp_premium", 0),
                "bp": req.get("bp_premium", 0)
            },
            "entry_premium": req.get("entry_premium", 0),
            "expiry": req.get("expiry", "10-Feb-2026")
        }
    else:
        position = {
            "strategy": "SHORT_STRADDLE",
            ...
            "premiums": {
                "ce": req.get("ce_premium", 0),
                "pe": req.get("pe_premium", 0)
            },
            "entry_premium": req.get("entry_premium", 0),
            "expiry": req.get("expiry", "10-Feb-2026")
        }
    ...
    telegram.send(f"📍 Position manually set:\n{strategy}\nEntry Premium: ₹{position['entry_premium']}")
